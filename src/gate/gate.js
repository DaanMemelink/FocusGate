...
function show(id) {
  // Hiding a <video> does not pause it. Without this the clip's audio keeps
  // playing over the puzzle, the intent box and the tally — the one thing the
  // wait screen must not do is follow you.
  if (id !== "step-wait") stopWaitMedia();
  for (const el of document.querySelectorAll(".step")) el.hidden = el.id !== id;
}

// Everything the wait screen leaves running. Called on every transition away
// from it, so advancing, bailing and clearing all get the same teardown.
function stopWaitMedia() {
  const media = $("clip-media");
  if (media) {
    media.pause();
    // Drop the source too, so a long clip is not left buffering behind a screen
  ...
  $("clip").hidden = true;
  $("unmute").hidden = true;
  // The quote rotation lives in the same timer list as the countdown, and the
  // countdown has already stopped itself by the time we can leave.
  clearTimers();
}
...
// gate is a page nobody clicked to reach — so a rejection here is the normal
// case, not an error. Try it anyway (some profiles allow it), and on refusal
// fall back to muted playback plus a one-click unmute: that click is exactly
// the gesture the policy is asking for.
//
// The button's visibility is driven off `media.muted` after the attempt rather
// than off which branch ran, so it is right whatever the browser decided.
async function startClip(media, unmute, wantSound) {
  media.muted = !wantSound;
  try {
    await media.play();
  } catch {
    media.muted = true;
    try {
      ...
      /* the clip simply will not play; the countdown is what matters */
    }
  }
  unmute.hidden = !media.muted;
}
